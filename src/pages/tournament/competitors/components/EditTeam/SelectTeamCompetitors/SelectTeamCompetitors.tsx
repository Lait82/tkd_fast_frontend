...
const SelectTeamCompetitors = ({}: SelectTeamCompetitorsProps) => {
    // ============ Deps ============
        ...
        return userCompetitors.filter(
            (userComp) =>
                !selectedMembers.some(
                    (selected) => selected === userComp.uuid
                ) &&
                !teamDraft.competitors.some(
                    (member) => member === userComp.uuid
                )
        );
    };

    const updateSelection = (selectedOption: CompetitorSchema) => {
        // Clears the previous selection and updates it

        const payload = [
            ...selectedMembers.filter((sm) => sm !== selected?.uuid),
        ];
        setSelectedMembers([...payload, selectedOption.uuid]);
    };
    ...
    const [selected, setSelected] = useState<CompetitorSchema | null>(); // State para uso interno del componente
